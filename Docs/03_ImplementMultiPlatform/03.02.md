---
title: '02. Connect the agent to the **Zava Retail Help Desk Bot** in Copilot Studio'
layout: default
nav_order: 2
parent: 'Exercise 03: Implement multi-platform agents solutions'
---

## Task 02: Connect the agent to the **Zava Retail Help Desk Bot** in Copilot Studio
...
1. Open **Zava Retail Help Desk Bot**.
...
1. Upon completion, you'll see this screen.

	![c9clwjk7.jpg](../../media/c9clwjk7.jpg)

1. In the **Azure AI Project Endpoint** field, enter the following value:

	```
    @lab.Variable(FoundryProjectEndpoint)
    ```
...
1. In the **Connect Microsoft Foundry agent** dialog, enter the following information:
   ...
   - **Description:**
	...
	```
	Use the zava‑store‑bank‑bot only when a customer inquiry requires verifying or referencing store‑specific banking information, including bank names, account types, routing numbers, pending transactions, or available balances linked to a specific Store ID. Provide very concise details exactly as listed in the agent's knowledge source, without inferring, altering, or expanding any financial data. This agent is intended strictly for internal validation and store‑related financial inquiries and must never be used for customer‑facing disclosures.
	```
