---
title: '01. Create a child agent to handle HR inquiries'
layout: default
nav_order: 1
parent: 'Exercise 03: Implement multi-platform agents solutions'
---

## Task 01: Create a child agent to handle HR inquiries
...
1. In the left pane, select **Agents**.
    ...
    ![di4k76kz.jpg](../../media/di4k76kz.jpg)

1. Select **Marketing-HR-Domain-Router**.
...
1. Select **Add and configure** then wait until the process completes. The MCP Server is added as a tool to your child agent.
...
1. On the command bar, select **Settings** and then **Connection Settings**.

1. For both **HRMCPServer** and **ZavaOpenAPICall**, select **Connect** and then select **Submit**.

1. Wait until the **Status** is **Connected** and then close the **Settings** page.
...
1. In the **Instructions** tile, select **Edit**. Search for and select **If HR, delegate to HRInquiryAgent.** 

1. Highlight and delete **HRInquiryAgent** and then enter **/**. Select the **HRInquiryAgent** agent from the list.

	![xontghn9.jpg](../../media/xontghn9.jpg)

1. On the **Instructions** pane, select **Save**. Then, on the command bar, select **Publish**.
